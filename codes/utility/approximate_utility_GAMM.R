...
log_posterior <- function(theta,X,Y,rand_u,rand_U ,z.spline,fishnet){
...
## function to approximate KLD utility for simulated parameters 
eval_utility <- function(X,Y,theta,rand_u,rand_U,z.spline,B1,fishnet){
  ...
  kld_beta_all <- kld_util(mu_prior[1:ncol(X)],as.matrix(sigma_prior[1:ncol(X),1:ncol(X)]),
                           mu_post[1:ncol(X)],as.matrix(sigma_post[1:ncol(X),1:ncol(X)]))
  kld_beta <- vector()
  for(i in 1:ncol(X)){
    kld_beta[i] <- kld_util(mu_prior[i],as.matrix(sigma_prior[i,i]),mu_post[i],as.matrix(sigma_post[i,i]))
  }
  kld_lsig_s <- kld_util(mu_prior[ncol(X)+1],as.matrix(sigma_prior[(ncol(X)+1),(ncol(X)+1)]),
                         mu_post[ncol(X)+1],as.matrix(sigma_post[(ncol(X)+1),(ncol(X)+1)]))
  kld_lsig_r <- kld_util(mu_prior[ncol(X)+2],as.matrix(sigma_prior[(ncol(X)+2),(ncol(X)+2)]),
                         mu_post[ncol(X)+2],as.matrix(sigma_post[(ncol(X)+2),(ncol(X)+2)]))
  out <- list(mu_post,sigma_post,kld,kld_beta,kld_beta_all,kld_lsig_s,kld_lsig_r)
...
exp_crit <- function(d,B){
  ...
  crit  <- foreach(j = 1:B,
                   ...
                   .export = ls(globalenv())) %dopar%  {
                     ...
                     post <- eval_utility(X=Xd_all,Y=HardCoral_all,theta=c(5,5,-1,-1),#theta_prior[j,],
                                          rand_u=rand_u,rand_U=rand_U,z.spline=z.spline_all,
                                          fishnet=fishnet_all)
