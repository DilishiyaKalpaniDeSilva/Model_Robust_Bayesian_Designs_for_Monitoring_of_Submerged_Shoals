...
indexT <- as.numeric(Sys.getenv("PBS_ARRAY_INDEX"))
indexD <- as.numeric(Sys.getenv("indexD")) ## number of designs
indexB <- as.numeric(Sys.getenv("indexB"))                    ## monte carlo utility
indexB1 <- as.numeric(Sys.getenv("indexB1"))                  ## monte carlo laplace
...
ntrans <- seq(from=18,to=2,by=-2)[indexT]
print(ntrans)

B_all <- c(500,250)
B1_all <- c(500,250)

B <- B_all[indexB] ;print(B)# no of MC for the utility approximation
B1 <- B1_all[indexB1] ;print(B1) # no of MC in Laplace approximation 

Sys.sleep((indexD+indexB+indexB1)*10)
...
U_path <- paste0("codes/utility/U_all",indexB,".RData")
...
tr_design <- design[[indexD]][1:ntrans]
...
if (!dir.exists(paste0("outputs/comparisons/run_time/",year,"/")) ) {
  dir.create(paste0("outputs/comparisons/run_time/",year,"/"), recursive = TRUE)
}

iname <- paste0("outputs/comparisons/run_time/",year,"/Design","_D",indexD,"_B_",B,"_B1_",B1,"_N",n_all,".RData")
