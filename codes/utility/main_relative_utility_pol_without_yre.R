...
## ---------------------------------------------------------------------------------------------
## Purpose:  Evaluate relative utility using polynomial prior without year random effect
## ---------------------------------------------------------------------------------------------
...
source("codes/utility/approximate_utility_pol_without_yre.R")
...
util <- vector()
for(i in 1:3){
  fol <- paste0("designs/polynomial/",year,"/",i,"/")
  iname <- paste0("outputs/",fol,"Optimal.RData")
  load(iname)
  util[i] <- mean(exp_crit(d=tr_design,B),na.rm=TRUE)
}

fol <- paste0("designs/GAMM/",year,"/")
iname <- paste0("outputs/",fol,"Optimal.RData")
load(iname)
util[4] <- mean(exp_crit(d=tr_design,B),na.rm=TRUE)

rel_util <- round(util/util[indexO],4)

if (!dir.exists(paste0("outputs/rel_utility/")) ) {
  dir.create(paste0("outputs/rel_utility/"), recursive = TRUE)
}

indexO_all <- c("Linear", "Quadratic", "Cubic")
path <- paste0("outputs/rel_utility/rel_util_", year, "_pol_", indexO_all[indexO], ".RData")
save(list=c("rel_util"), file=path)
