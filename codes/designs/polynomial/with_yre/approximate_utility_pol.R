...
log_posterior <- function(theta,X,Y,year,rand_U,rand_t,fishnet){
  ...
  pt <- dlgamma(theta[indexO+3],location=-21,scale = 5, shape = 10,log = TRUE) 
  log_prior <- pb + pr
  
  U <- qnorm(rand_U,0, 1/(sqrt(exp(theta[indexO+2]))) )
  tre <- qnorm(rand_t,0, 1/(sqrt(exp(theta[indexO+3]))) )
  tre1 <- rbind(0,tre)
  
  rand <- U[fishnet,] + tre1[year,]
...
eval_utility <- function(X,Y,theta,year,rand_U,rand_t,B1,fishnet){
  ...
  fitOptim_post <- optim(par=theta, fn =log_posterior, method = "L-BFGS-B",
                         ...
                         X=X,rand_U=rand_U,Y=Y,fishnet=fishnet,year=year, rand_t=rand_t
  )
...
exp_crit <- function(d,B){
  ...
  crit  <- foreach(j = 1:B,
                   ...
                   .export = ls(globalenv())) %dopar%  {
                     ...
                     #t1 <- Sys.time()
                     util <- eval_utility(X=Xd_all,Y=HardCoral_all,theta=c(rep(5, indexO+1),-1,-1),
                                          rand_U=rand_U,fishnet=fishnet_all, rand_t=rand_t, year=y_all)
                     
                     util 
                   }
  crit
}
# X=Xd_all; Y=HardCoral_all; theta=c(rep(5, indexO+1),-1); rand_U=rand_U; fishnet=fishnet_all
