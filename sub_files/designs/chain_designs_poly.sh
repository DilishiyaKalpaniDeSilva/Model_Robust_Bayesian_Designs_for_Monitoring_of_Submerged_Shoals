...
# dpath="polynomial/with_yre"     # uncomment this to run polynomial designs with year random effect
dpath="polynomial/without_yre"    # uncomment this to run polynomial designs without year random effect
...
for indY in 1; do     # index for prior data set
  for indO in 2;do     # index for degree of polynomial
    
    log_path="${log_path1}/${indY}/${indO}/${indI}/"
    
    # Initial job submission for initialization
    init=$(qsub -N init_util_Y${indY} -v log_path=$log_path,ncl=$ncl,indexY=$indY,indexO=$indO,indexI=$indI,B=$B,B1=$B1,path=$path0 $sub_init)
    echo "Initial job submitted with ID: $init"
    
    for iter in 1; do     # loop over itertaions
      for indT in {1..2}; do      # loop over transects
          
        jname_t="Y${indY}_I${indI}_T${indT}_Iter${iter}"
        common_params="log_path=$log_path,ncl=$ncl,indexI=$indI,indexY=$indY,indexO=$indO,iter=$iter,indexTr=$indT,B=$B,B1=$B1"
      ...
      # check optimality at the end of each iteration
